...
const LoginPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Redirect to default route after 3 seconds
                setTimeout(() => {
                    navigate('/');
                }, 3000);
            }
    ...
    return (
        ...
        >
            ...
            {validationErrors.length >= 1 && <ToastMessage error={validationErrors} />}
        </div>
    );
}
...
const ToastMessage = ({error}) => {
    ...
    return (
        <div className="flex items-center justify-center gap-3 text-red-900 font-medium w-fit h-fit bg-red-400 rounded-md 
                        z-100 mt-[-10px] py-3 px-10">
            <OctagonAlert />
            {error.map((e, i) => (
                <p key={i}>{e}</p>
            ))}
